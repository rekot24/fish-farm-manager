...
| [`bot/`](bot/) | The core: device lifecycle (`device_manager.py`), the per-device scan/detect/act loop (`device_worker.py`), state resolution (`state_machine.py`, `state_rules.py`, `states.py`), ADB input actions (`actions.py`), inter-device signaling (`farm_event_bus.py`), and battery/temp/ADB health checks (`health_monitor.py`). |
...
| [`config/`](config/) | All configuration. `settings.json` (global settings, not committed — see `.gitignore`), `devices.json` (per-device config, not committed), `profiles/*.yaml` (per-role state-detection rule sets and behaviors — `lead_private`, `support_private`, `lead_public`, `support_public`). Loaded/saved/validated by `bot/config_manager.py`. |
