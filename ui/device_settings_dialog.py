...
class DeviceSettingsDialog:
    ...
    def __init__(self, parent, device_cfg: DeviceConfig, all_devices: List[DeviceConfig]):
        ...
        self._auto_timer = tk.IntVar(value=device_cfg.timers.auto_farm_reset_interval_min)
        self._end_timer = tk.IntVar(value=device_cfg.timers.end_run_reset_interval_min)
    ...
    def _build(self) -> None:
        ...
        # ---- Identity ----
        ttk.Label(outer, text="Identity", font=("TkDefaultFont", 9, "bold")).pack(anchor="w")

        row("Nickname:", lambda f: ttk.Entry(f, textvariable=self._nickname, width=28))
        row("Notes:", lambda f: ttk.Entry(f, textvariable=self._notes, width=40))

        # Serial (read-only)
        f = ttk.Frame(outer)
        f.pack(fill="x", pady=3)
        ttk.Label(f, text="Serial:", width=24, anchor="w").pack(side="left")
        ...
        ttk.Label(outer, text="Role & Profile", font=("TkDefaultFont", 9, "bold")).pack(anchor="w")

        row("Profile:", lambda f: ttk.Combobox(
            f, textvariable=self._profile, values=PROFILES, state="readonly", width=20))

        f = ttk.Frame(outer)
        f.pack(fill="x", pady=3)
        ttk.Label(f, text="Is Lead Device:", width=24, anchor="w").pack(side="left")
        ...
        # ---- Capture & Scan ----
        ttk.Separator(outer, orient="horizontal").pack(fill="x", pady=(10, 4))
        ttk.Label(outer, text="Capture & Scan", font=("TkDefaultFont", 9, "bold")).pack(anchor="w")

        row("Capture Backend:", lambda f: ttk.Combobox(
            f, textvariable=self._backend, values=BACKENDS, state="readonly", width=12))
        row("Scan Interval (ms):", lambda f: ttk.Spinbox(
            f, from_=200, to=5000, increment=100, textvariable=self._scan_ms, width=8))

        # ---- Timers ----
        ttk.Separator(outer, orient="horizontal").pack(fill="x", pady=(10, 4))
        ttk.Label(outer, text="Timers", font=("TkDefaultFont", 9, "bold")).pack(anchor="w")

        row("Auto-Farm Reset (min):", lambda f: ttk.Spinbox(
            f, from_=1, to=60, textvariable=self._auto_timer, width=6))
        row("End-Run Reset (min):", lambda f: ttk.Spinbox(
            f, from_=1, to=120, textvariable=self._end_timer, width=6))

        # ---- Tools ----
        ttk.Separator(outer, orient="horizontal").pack(fill="x", pady=(10, 4))
        ttk.Label(outer, text="Setup Tools", font=("TkDefaultFont", 9, "bold")).pack(anchor="w")

        tools_row = ttk.Frame(outer)
        tools_row.pack(anchor="w", pady=4)
        ttk.Button(tools_row, text="Open Image Capture Tool",
                   command=self._open_capture_tool).pack(side="left", padx=(0, 8))
    ...
    def _save(self) -> None:
        ...
        self.result = DeviceConfig(
            ...
            timers=TimerConfig(
                auto_farm_reset_interval_min=self._auto_timer.get(),
                end_run_reset_interval_min=self._end_timer.get(),
            ),
    ...
    def _open_capture_tool(self) -> None:
        """Launch the Image Capture Tool for this device."""
        # Deferred import to avoid circular dependency
        from tools.image_capture_tool import ImageCaptureTool
        ImageCaptureTool(self.top, device_cfg=self.result)
